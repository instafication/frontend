...
export const appRouter = t.router({
	...
	getConfiguration: t.procedure
		...
		.query(async ({ input }) => {
			...
			const serviceConfiguration: Service = await DatabaseManager.Services.getServiceConfiguration(
				user,
				name
			);
	...
	removeService: t.procedure
		...
		.query(async ({ input }) => {
			const { user, name } = input;
			const isRemoved: boolean = await DatabaseManager.Services.removeServiceNameByUserId(
				user,
				name
			);
			return isRemoved;
	...
	raw_user_meta_data: t.procedure.input(z.string()).query(async ({ input }) => {
	...
	getLastUpdateByCompanyName: t.procedure.input(z.string()).query(async ({ input }) => {
		const company = input;
