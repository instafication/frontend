...
export const scraper_GetLastUpdateByCompanyName = query(
	v.string(),
	async (companyName: string): Promise<number> =>
		await db()
			.select({ last_update: scrapers.last_update })
			.from(scrapers)
...
export const profile_EnsureExistsByUserId = command(v.string(), async (userId: string) => {
	...
	const res = await db().insert(profiles).values({ id: userId, credits: 3 });
	return res.success;
});
...
export const profile_GetCreditsByUserId = query(v.string(), async (userId: string) => {
	...
	if (typeof credits !== 'number' || credits < 0) {
		try {
			await db().update(profiles).set({ credits: 3 }).where(eq(profiles.id, userId));
		} catch (e) {
...
export const profile_GetUserEmailByUserId = query(
	v.string(),
	async (userId: string) =>
		await db()
...
export const profile_GetUserByUserId = query(
	v.string(),
	async (id) =>
		await db()
			.select()
...
export const profile_GetUsersWithCredits = query(
	async () => await db().select().from(profiles).where(gt(profiles.credits, 0))
);
...
export const profile_GetAll = query(async () => await db().select().from(profiles));

export const profile_Create = command(
	ProfileInsertSchema,
	async (payload: ProfileInsert) =>
		await db()
			.insert(profiles)
			.values({
				...payload,
...
export const profile_UpdateUserByUserId = command(
	v.object({ userId: v.string(), data: ProfileUpdateSchema }),
	async ({ userId, data }: { userId: string; data: ProfileUpdate }) =>
		await db()
			.update(profiles)
			.set(data)
			.where(eq(profiles.id, userId))
...
export const profile_RemoveUserByUserId = command(
	v.string(),
	async (userId: string) => await db().delete(profiles).where(eq(profiles.id, userId))
);

export const profile_DecreaseOneCreditByUserId = command(
	v.string(),
	async (userId: string) =>
		await db()
			.update(profiles)
...
export const profile_ProlongSubscriptionByUserEmail = command(
...
export const profile_RefillCreditsByUserEmail = command(
...
export const notification_Create = command(
	NotificationInsertSchema,
	async (payload: NotificationInsert) =>
		await db()
...
export const notification_GetByArea = query(
	v.string(),
	async (area: string) =>
		await db().select().from(notifications).where(eq(notifications.area, area))
);

export const notification_GetAll = query(async () => await db().select().from(notifications));

export const notification_GetLatest = query(
	v.optional(v.number(), 3),
	async (count = 3) =>
		await db().select().from(notifications).orderBy(desc(notifications.date)).limit(count)
);

/*──────────────────────── SERVICES ──────────────────────*/
export const service_GetConfigByCompanyName = query(
	v.object({ companyName: v.string(), userId: v.optional(v.string()) }),
...
export const service_RemoveByUserIdAndServiceName = command(
	v.object({ userId: v.string(), serviceName: v.string() }),
